...
void print_ttl_group(struct ping_context *ctx) {
    char *curr_addrr = "";

    printf("%d ", ctx->curr_ttl_to_print + 1);
    for (uint8_t i = 0; i < MAX_PROBES; i++) {

        probe_record cur = ctx->probes[ctx->curr_ttl_to_print][i];
...
bool is_ttl_group_ready(const struct ping_context *ctx) {
    for (uint8_t i = 0; i < MAX_PROBES; i++) {
...
void print_ready_ttl_groups(struct ping_context *ctx) {
    while (is_ttl_group_ready(ctx)) {
        ...
        if (ctx->curr_ttl_to_print > ctx->final_ttl) {
            cleanup(ctx);
            exit(EXIT_SUCCESS);
        }
    }
